...
def doctor_home(request):
	...
	medical1 = Medical.objects.filter(medicine='Call / Discussion With Doctor').count()
...
def MakeMend(request):
  ...
  if sick in disease_list:
      ...
      Medical.objects.filter(pk= disease).update(medicine='Call / Discussion With Doctor')
